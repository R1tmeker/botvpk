...
def _parse_bool(value: str) -> bool:
    ...
    if normalized in {"нет", "false", "0", "no", "n"}:
        return False
    raise ValueError("ожидал да/нет")
...
async def add_poll_options(message: Message, state: FSMContext) -> None:
    ...
    if len(options) < 2:
        await message.answer("Введите минимум два варианта через |.")
        return
    await state.update_data(options=options)
    await state.set_state(AddPollStates.is_anonymous)
    await message.answer("Шаг 4/10. Анонимный опрос? (да/нет)")
...
async def add_poll_is_anonymous(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Шаг 5/10. Разрешить несколько ответов? (да/нет)")
...
async def add_poll_schedule_type(message: Message, state: FSMContext) -> None:
    ...
    if schedule_type not in {"weekly", "daily", "once"}:
        await message.answer("Используйте weekly, daily или once.")
        return
    ...
    if schedule_type == "weekly":
        ...
        await message.answer("Шаг 7/10. Укажите время HH:MM (локально).")
...
async def add_poll_days(message: Message, state: FSMContext) -> None:
    ...
    if not days:
        await message.answer("Укажите хотя бы один день.")
        return
    await state.update_data(days=days)
    await state.set_state(AddPollStates.time_local)
    await message.answer("Шаг 8/10. Укажите время HH:MM (локально).")


@router.message(AddPollStates.time_local)
async def add_poll_time(message: Message, state: FSMContext) -> None:
    try:
        hour, minute = map(int, message.text.strip().split(":"))
        time_local = time(hour=hour, minute=minute)
    except Exception:
        await message.answer("Формат времени: HH:MM (например, 19:30).")
        return
    await state.update_data(time_local=time_local)
    await state.set_state(AddPollStates.target_chat)
    await message.answer("Шаг 9/10. ID чата (оставьте пустым для значения по умолчанию).")
...
async def add_poll_target_chat(message: Message, state: FSMContext, context: BotContext) -> None:
    ...
    if text:
        try:
            ...
            await message.answer("Введите целое число или оставьте поле пустым.")
            return
    else:
        chat_id = context.config.default_poll_chat_id
        if not chat_id:
            await message.answer("Значение по умолчанию не задано. Введите ID чата.")
            return
    await state.update_data(target_chat_id=chat_id)
    await state.set_state(AddPollStates.thread_id)
    await message.answer(
        "Шаг 10/10. ID топика (если нужно). Оставьте пустым, если не используется."
    )
...
async def add_poll_thread_id(message: Message, state: FSMContext) -> None:
    ...
    if text:
        try:
            ...
            await message.answer("Введите числовой ID или оставьте пустым.")
            return
    else:
        thread_id = None
    ...
    preview = [
        ...
        f"Дни: {', '.join(data.get('days', [])) if data.get('days') else '-'}",
...
async def list_polls(message: Message, member, context: BotContext) -> None:
    ...
    for poll in polls:
        ...
        lines.append(
            f"{poll.poll_id}: {poll.title} ({poll.schedule_type} {days} {poll.time_local.strftime('%H:%M')}) — {'Вкл' if poll.is_active else 'Выкл'}"
        )
    await message.answer("\n".join(lines))
...
async def toggle_poll(message: Message, member, context: BotContext, command: CommandObject) -> None:
    ...
    args = command.args
    if not args:
        await message.answer("Использование: /toggle_poll <id>")
        return
    try:
        poll_id = int(args.split()[0])
    except ValueError:
...
async def delete_poll(message: Message, member, context: BotContext, command: CommandObject) -> None:
    ...
    args = command.args
    if not args:
        await message.answer("Использование: /delete_poll <id>")
        return
    try:
        poll_id = int(args.split()[0])
    except ValueError:
...
async def edit_poll_start(message: Message, member, context: BotContext, command: CommandObject, state: FSMContext) -> None:
    if not await ensure_role(message, getattr(member, "role", None), ADMIN_ROLES):
        return
    args = command.args
    if not args:
        await message.answer("Использование: /edit_poll <id>")
        return
    try:
        poll_id = int(args.split()[0])
    except ValueError:
    ...
    await message.answer(
        "Отправьте изменения в формате field=value построчно. Допустимые поля: title, question, options, "
        "is_anonymous, allows_multiple_answers, schedule_type, days, time_local, target_chat_id, message_thread_id. "
...
async def edit_poll_process(message: Message, state: FSMContext, context: BotContext) -> None:
    ...
    if text.lower() == "готово":
        ...
        await message.answer(f"Обновление завершено. Текущий статус: {'вкл' if poll.is_active else 'выкл'}.")
        await state.clear()
        return

    if "=" not in text:
        await message.answer("Используйте формат field=value или 'готово'.")
        return
    field, value = [part.strip() for part in text.split("=", 1)]
    data = await state.get_data()
    poll = context.poll_service.get_poll(data["poll_id"])
    try:
        updated = _apply_field(poll, field, value)
        context.poll_service.save_poll(updated)
        await context.poll_scheduler.refresh()
        ...
        await message.answer(f"Формат данных: {exc}")
...
def _apply_field(poll, field: str, value: str) -> PollInput:
    ...
    if field == "title":
        ...
        if len(options) < 2:
            raise ValidationServiceError("Минимум два варианта.")
        ...
        if value_lower not in {"weekly", "daily", "once"}:
            raise ValidationServiceError("schedule_type: weekly/daily/once")
        ...
        try:
            ...
            raise ValueError("time_local=HH:MM") from exc
